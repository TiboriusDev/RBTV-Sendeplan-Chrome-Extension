    ...
    data.data.forEach(days => {
        ...
        if (sendeDate[0] == dayNow) {
            days.elements.forEach(sendung => {
                ...
                let vod = (sendung.type != "rerun") ? '' : '<a href="https://rocketbeans.tv/mediathek/video/' + sendung.episodeId + '" target="_blank"><div class="vod"><img src="images/svg/yt.svg"></div></a></div>';

                if (sendung.channelGroups.type != "talent" && (new Date()).getTime() > new Date(sendung.timeStart).getTime() && (new Date()).getTime() < new Date(sendung.timeEnd).getTime()) {
